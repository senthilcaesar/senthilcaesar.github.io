        (function () {
            ...
            weatherOptions.forEach(opt => {
                opt.addEventListener('click', (e) => {
                    ...
                    setWeather(selected, opt.querySelector('.option-icon').textContent, opt.querySelector('.option-label').textContent);
                });
            });
            ...
            function runSimulation(mode) {
                ...
                document.body.classList.remove('weather-sun-rising', 'weather-summer', 'weather-cloudy', 'weather-windy', 'weather-fall', 'weather-moon');
            ...
            class Drop {
                ...
                reset(isInitial = false, customX = null, customY = null, customVx = null, customVy = null) {
                    ...
                    if (activeGears && activeGears.length > 0) {
                        for (let gear of activeGears) {
                            ...
                            if (dist <= gear.radius && dy < 0 && bottomPrevY <= gear.cy) {
                                ...
                                gearDroplets.push(new GearDroplet(gear.index, hitAngle, gear.radius, this.width * 1.5));
                                return false;
                            }
                        }
                    }
                    ...
                    if (activeObstacles && activeObstacles.length > 0) {
                        for (let obs of activeObstacles) {
                            if (bottomPrevY <= obs.top && bottomNextY >= obs.top) {
                                const t = (obs.top - bottomPrevY) / (bottomNextY - bottomPrevY || 1);
                                const hitX = prevX + (this.x - prevX) * t;
                                if (hitX >= obs.left && hitX <= obs.right) {
                                    createSplash(hitX, obs.top);
                                    if (Math.random() < 0.15) {
                                        cardDroplets.push(new CardDroplet(hitX, obs.top, obs.bottom, this.width * 1.4));
                                    }
                                    return false;
                                }
                            }
            ...
            class GearDroplet {
                ...
                draw() {
                    ...
                    const isDark = document.body.classList.contains('dark-mode');
                    ctx.strokeStyle = isDark ? `rgba(165, 203, 255, 0.8)` : `rgba(47, 65, 90, 0.75)`;
                    ...
                    ctx.fillStyle = isDark ? `rgba(200, 225, 255, 0.95)` : `rgba(60, 80, 110, 0.9)`;
                    ctx.beginPath();
            ...
            class CardDroplet {
                ...
                draw() {
                    if (this.trail.length < 2) return;
                    const isDark = document.body.classList.contains('dark-mode');
                    ctx.strokeStyle = isDark ? `rgba(165, 203, 255, 0.45)` : `rgba(47, 65, 90, 0.4)`;
                    ctx.lineWidth = this.size;
                    ctx.lineCap = 'round';
                    ctx.beginPath();
                    ctx.moveTo(this.trail[0].x, this.trail[0].y);
                    ...
                    const head = this.trail[this.trail.length - 1];
                    ctx.fillStyle = isDark ? `rgba(180, 215, 255, 0.7)` : `rgba(50, 70, 95, 0.65)`;
                    ctx.beginPath();
            ...
            class Snowflake {
                ...
                update(activeObstacles, activeGears) {
                    ...
                    if (activeGears && activeGears.length > 0) {
                        for (let gear of activeGears) {
                            ...
                            if (dist <= gear.radius && dy < 0 && (this.y - this.speed) <= gear.cy) {
                                const hitAngle = Math.atan2(dy, dx);
                                gearSnowlets.push(new GearSnowlet(gear.index, hitAngle, gear.radius, this.radius));
                                return false;
                            }
                        }
                    }
                    ...
                    if (activeObstacles && activeObstacles.length > 0) {
                        for (let obs of activeObstacles) {
                            if (this.y >= obs.top && (this.y - this.speed) <= obs.top && this.x >= obs.left && this.x <= obs.right) {
                                this.settled = true;
                                this.y = obs.top - 0.5;
                                return true;
                            }
            ...
            class Leaf {
                ...
                reset(isInitial = false) {
                    ...
                    this.colorTemplate = colorsList[Math.floor(Math.random() * colorsList.length)];
                ...
                update(activeObstacles) {
                    ...
                    if (activeObstacles && activeObstacles.length > 0) {
                        for (let obs of activeObstacles) {
                            if (this.y >= obs.top && (this.y - this.vy) <= obs.top && this.x >= obs.left && this.x <= obs.right) {
                                this.settled = true;
                                this.y = obs.top - 1;
                                this.vx = 0;
                                this.vy = 0;
                ...
                draw() {
                    const curOpacity = this.settled 
                        ? this.opacity * (this.settleAge / this.maxSettleAge) 
                        : this.opacity;
                    ctx.fillStyle = this.colorTemplate.replace('opacity', curOpacity.toString());
            ...
            class WindStreak {
                ...
                draw() {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(this.x, this.y);
                    ctx.quadraticCurveTo(this.x + this.length / 2, this.y + (Math.sin(this.x * 0.01) * 15), this.x + this.length, this.y);
                    ctx.stroke();
                }
            }
            ...
            class AmberSparkle {
                ...
                update() {
                    ...
                    if (this.x < -10 || this.x > width + 10 || this.y < -10 || this.age > this.maxAge) {
                        return false;
                    }
                    return true;
                }
                ...
                draw() {
                    ...
                    if (this.type === 'summer') {
                        ctx.fillStyle = isDark ? `rgba(253, 224, 71, ${curOpacity})` : `rgba(245, 158, 11, ${curOpacity})`;
                        ctx.shadowColor = 'rgba(245, 158, 11, 0.5)';
                    } else if (this.type === 'sunset') {
                        ctx.fillStyle = isDark ? `rgba(232, 121, 249, ${curOpacity})` : `rgba(219, 39, 119, ${curOpacity * 0.8})`;
                        ctx.shadowColor = 'rgba(219, 39, 119, 0.5)';
                    } else { // sunrise
                        ctx.fillStyle = isDark ? `rgba(251, 191, 36, ${curOpacity})` : `rgba(239, 68, 68, ${curOpacity * 0.8})`;
            ...
            class MatrixCode {
                ...
                update() {
                    ...
                    if (now - this.lastUpdate > this.updateInterval) {
                        ...
                        const characters = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍｦｲｸｺｿﾁﾄﾉﾌﾔﾖﾙﾚﾛﾝ";
            ...
            function resizeCanvas() {
                ...
                CONFIG.maxDrops = Math.min(Math.floor(width * 0.15 * densityMultiplier), isMobile ? 80 : 250);
                CONFIG.maxSnowflakes = Math.min(Math.floor(width * 0.12 * densityMultiplier), isMobile ? 70 : 220);
                CONFIG.maxLeaves = Math.min(Math.floor(width * 0.04 * densityMultiplier), isMobile ? 25 : 70);
            ...
            function initEntities(mode) {
                ...
                } else if (mode === 'sun-rising') {
                    for (let i = 0; i < CONFIG.maxSparkles * 0.5; i++) {
                        sparkles.push(new AmberSparkle('sunrise'));
                    }

                } else if (mode === 'summer') {
                    for (let i = 0; i < CONFIG.maxSparkles * 0.5; i++) {
                        sparkles.push(new AmberSparkle('summer'));
                    }
                } else if (mode === 'windy') {
            ...
            function loop() {
                ...
                const activeGears = [];
                if (window.gearsData && window.gearsData.camera && window.gearsData.gears && typeof THREE !== 'undefined' && cachedGearContainer) {
                    const containerRect = {
                        ...
                        right: cachedGearContainer.pageLeft - scrollX + cachedGearContainer.width,
                        bottom: cachedGearContainer.pageTop - scrollY + cachedGearContainer.height
                    };
                    
                    if (containerRect.bottom > 0 && containerRect.top < height && containerRect.right > 0 && containerRect.left < width) {
                        window.gearsData.gears.forEach((gear, idx) => {
                            ...
                            const cx = containerRect.left + (wp.x * 0.5 + 0.5) * containerRect.width;
                            const cy = containerRect.top + (-(wp.y * 0.5) + 0.5) * containerRect.height;
                            ...
                            const px = containerRect.left + (localPoint.x * 0.5 + 0.5) * containerRect.width;
                            const py = containerRect.top + (-(localPoint.y * 0.5) + 0.5) * containerRect.height;
                ...
                if (currentMode === 'rain') {
                    ...
                    // Shimmering photons
                    while (sparkles.length < CONFIG.maxSparkles) {
                        sparkles.push(new AmberSparkle('sunrise'));
                    }
                    ...
                    const gradient = ctx.createRadialGradient(width, 0, 0, width, 0, Math.max(width, height) * 0.8 * pulse);
